___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Cookies and Click IDs extractor and consolidator",
  "brand": {
    "id": "brand_dummy",
    "displayName": "",
    "thumbnail": "data:image/png;base64,\u003d\u003d"
  },
  "description": "The tag allows parsing query parameters and cookie names using flexible, pre-defined lists of keys. It supports both \u0027first\u0027 and \u0027last\u0027 attribution modes, and provides detailed logs for fast debugging",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "TEXT",
    "name": "cookiesList",
    "displayName": "Cookie names separated by comma",
    "simpleValueType": true,
    "alwaysInSummary": true,
    "help": "For example: _scid,_twclid,_rdt_uuid"
  },
  {
    "type": "TEXT",
    "name": "clicksList",
    "displayName": "URL query parameters separated by comma",
    "simpleValueType": true,
    "alwaysInSummary": true,
    "help": "For example: ScCid,twclid,rdt_cid"
  },
  {
    "type": "CHECKBOX",
    "name": "firstClickAttribution",
    "checkboxText": "First click",
    "simpleValueType": true,
    "alwaysInSummary": true,
    "defaultValue": true,
    "displayName": "The attribution-type cookie should be set"
  },
  {
    "type": "CHECKBOX",
    "name": "lastClickAttribution",
    "checkboxText": "Last click",
    "simpleValueType": true,
    "alwaysInSummary": true,
    "defaultValue": true
  },
  {
    "type": "TEXT",
    "name": "initialCookieLifetime",
    "displayName": "First click cookies life time (days)",
    "simpleValueType": true,
    "alwaysInSummary": true,
    "valueValidators": [
      {
        "type": "POSITIVE_NUMBER"
      }
    ]
  },
  {
    "type": "TEXT",
    "name": "lastCookieLifetime",
    "displayName": "Last cookies life time (days)",
    "simpleValueType": true,
    "alwaysInSummary": true,
    "valueValidators": [
      {
        "type": "POSITIVE_NUMBER"
      }
    ]
  },
  {
    "type": "CHECKBOX",
    "name": "enableLogs",
    "checkboxText": "Enable logs to console",
    "simpleValueType": true,
    "alwaysInSummary": true,
    "defaultValue": true,
    "displayName": "Quality control"
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const log = require('logToConsole');
const getCookieValues = require('getCookieValues');
const setCookie = require('setCookie');
const getTimestamp = require('getTimestamp');
const getQueryParameters = require('getQueryParameters');
const makeNumber = require('makeNumber');

var enableLogs = data.enableLogs;
var firstClickAttribution = data.firstClickAttribution;
var lastClickAttribution = data.lastClickAttribution;

var initialCookieLifetime = makeNumber(data.initialCookieLifetime);
var lastCookieLifetime = makeNumber(data.lastCookieLifetime);


// Helper function for conditional logging
function conditionalLog(message) {
    if (enableLogs) {
        log(message);
    }
}

conditionalLog('The script to set cookies for CAPI is in progress. Attribution selected: ' + "First click is " + firstClickAttribution + "; and Last click is " + lastClickAttribution);


conditionalLog('The initial cookie life time is ' + initialCookieLifetime + ' days; and the last cookie life time is ' + lastCookieLifetime + ' days');

// Retrieve cookie and query parameter lists
function retrieveLists() {
    conditionalLog('Retrieving the list of cookies and clicks.');
    var cookiesList = data.cookiesList; // A string with comma-separated cookie names
    var clicksList = data.clicksList; // A string with comma-separated query param names

    conditionalLog('Cookies List: ' + cookiesList);
    conditionalLog('Clicks List: ' + clicksList);

    return {
        cookiesList: cookiesList.split(','),
        clicksList: clicksList.split(',')
    };
}

// Retrieve existing initial_u_ids and initial_c_ids cookie values
function retrieveExistingInitialValues() {

    conditionalLog('Checking for existing initial_u_ids cookie...');
    var existingInitialMarketingIds = getCookieValues('initial_u_ids');
    conditionalLog('Existing initial_u_ids value: ' + existingInitialMarketingIds);

    conditionalLog('Checking for existing initial_c_ids cookie...');
    var existingInitialQueryParams = getCookieValues('initial_c_ids');
    conditionalLog('Existing initial_c_ids value: ' + existingInitialQueryParams);

    return {
        existingInitialMarketingIds: existingInitialMarketingIds,
        existingInitialQueryParams: existingInitialQueryParams
    };
}

// Retrieve existing last_u_ids and last_c_ids cookie values
function retrieveExistingLastValues() {


    conditionalLog('Checking for existing last_u_ids cookie...');
    var existingLastMarketingIds = getCookieValues('last_u_ids');
    conditionalLog('Existing last_u_ids value: ' + existingLastMarketingIds);


    conditionalLog('Checking for existing last_c_ids cookie...');
    var existingLastQueryParams = getCookieValues('last_c_ids');
    conditionalLog('Existing last_c_ids value: ' + existingLastQueryParams);

    return {
        existingLastMarketingIds: existingLastMarketingIds,
        existingLastQueryParams: existingLastQueryParams
    };
}

// Process cookie data for initial_u_ids
function processInitialMarketingCookieIds(existingInitialMarketingIds, cookiesList) {
    if (existingInitialMarketingIds == '') {
        conditionalLog('No existing initial_u_ids found. Proceeding to fetch cookies.');
        var newMarketingIds = buildNewCookieData(cookiesList);
        if (newMarketingIds) {
            setCookie('initial_u_ids', newMarketingIds, {
                'max-age': initialCookieLifetime * 24 * 60 * 60,
                'domain': 'auto'
            }, false);

            conditionalLog('New initial_u_ids set successfully.');
        } else {
            conditionalLog('No cookie values found in cookiesList. initial_u_ids will not be set.');
        }
    } else {
        conditionalLog('Existing initial_u_ids found. Proceeding to update it.');
        var updatedInitialMarketingIds = updateExistingInitialCookieData(existingInitialMarketingIds, cookiesList);
        if (updatedInitialMarketingIds) {
            setCookie('initial_u_ids', updatedInitialMarketingIds, {
                'max-age': initialCookieLifetime * 24 * 60 * 60,
                'domain': 'auto'
            }, false);
            conditionalLog('Updated initial_u_ids set successfully.');
        } else {
            conditionalLog('No updates made to initial_u_ids. No values found.');
        }
    }
}

// Process cookie data for last_u_ids
function processLastMarketingCookieIds(existingLastMarketingIds, cookiesList) {
    if (existingLastMarketingIds == '') {
        conditionalLog('No existing last_u_ids found. Proceeding to fetch cookies.');
        var newMarketingIds = buildNewCookieData(cookiesList);
        if (newMarketingIds) {
            setCookie('last_u_ids', newMarketingIds, {
                'max-age': lastCookieLifetime * 24 * 60 * 60,
                'domain': 'auto'
            }, false);
            conditionalLog('New last_u_ids set successfully.');
        } else {
            conditionalLog('No cookie values found in cookiesList. last_u_ids will not be set.');
        }
    } else {
        conditionalLog('Existing last_u_ids found. Proceeding to update it.');
        var updatedLastMarketingIds = updateExistingLastCookieData(existingLastMarketingIds, cookiesList);
        if (updatedLastMarketingIds) {
            setCookie('last_u_ids', updatedLastMarketingIds, {
                'max-age': lastCookieLifetime * 24 * 60 * 60,
                'domain': 'auto'
            }, false);
            conditionalLog('Updated last_u_ids set successfully.');
        } else {
            conditionalLog('No updates made to last_u_ids. No values found.');
        }
    }
}

// Build new u_ids string
function buildNewCookieData(cookiesList) {
    var newMarketingIds = '';
    for (var i = 0; i < cookiesList.length; i++) {
        var cookieKey = cookiesList[i].trim();
        conditionalLog('Fetching value for cookie: ' + cookieKey);
        var cookieValue = getCookieValues(cookieKey);
        conditionalLog('Retrieved value for cookie ' + cookieKey + ': ' + cookieValue);

        if (cookieValue == '') {

            conditionalLog('The cookie ' + cookieKey + ' will be used as null here, because its value is empty');
            if (newMarketingIds) {
                newMarketingIds += '|';
            }
            newMarketingIds += cookieKey + '=' + cookieValue;

        }

        if (cookieValue != '') {
            if (newMarketingIds) {
                newMarketingIds += '|';
            }
            newMarketingIds += cookieKey + '=' + cookieValue;
        }
    }
    return newMarketingIds || null;
}

// Update existing initial_u_ids with new values
function updateExistingInitialCookieData(existingInitialMarketingIds, cookiesList) {

    conditionalLog('We starting to parse existing initial_u_ids: ' + existingInitialMarketingIds);
    var updatedMarketingMap = parseCookieData(existingInitialMarketingIds);
    conditionalLog('We have parsed existing initial_u_ids: ' + existingInitialMarketingIds);
    var newCookieData = '';
    var updateCookiesFlag = false;

    for (var i = 0; i < cookiesList.length; i++) {
        var cookieKey = cookiesList[i].trim();
        conditionalLog('Fetching value for cookie: ' + cookieKey);
        var cookieValue = getCookieValues(cookieKey);
        conditionalLog('Retrieved value for cookie ' + cookieKey + ': ' + cookieValue);

        if (cookieValue == '') {

            conditionalLog('The cookie ' + cookieKey + ' will be used as null here, because its value is empty');


        }

        if (cookieValue != '') {


            if (updatedMarketingMap[cookieKey] == '' || !updatedMarketingMap[cookieKey]) {

                updateCookiesFlag = true;
                updatedMarketingMap[cookieKey] = cookieValue;
                conditionalLog('Updating/Adding key: ' + cookieKey + ' with value: ' + cookieValue);

            } else {

                conditionalLog('The cookie ' + cookieKey + ' will not be updated, because of initial atribtuion type. The previous value is not empty.');

            }

        }
    }

    if (updateCookiesFlag == true) {
        // Rebuild the initial_u_ids string
        for (var key in updatedMarketingMap) {
            if (updatedMarketingMap.hasOwnProperty(key)) {
                if (newCookieData) {
                    newCookieData += '|';
                }
                newCookieData += key + '=' + updatedMarketingMap[key];
            }
        }
    }

    return newCookieData || null;
}

// Update existing last_u_ids with new values
function updateExistingLastCookieData(existingLastMarketingIds, cookiesList) {

    conditionalLog('We starting to parse existing last_u_ids: ' + existingLastMarketingIds);
    var updatedMarketingMap = parseCookieData(existingLastMarketingIds);
    conditionalLog('We have parsed existing last_u_ids: ' + existingLastMarketingIds);
    var newCookieData = '';
    var updateCookiesFlag = false;

    for (var i = 0; i < cookiesList.length; i++) {
        var cookieKey = cookiesList[i].trim();
        conditionalLog('Fetching value for cookie: ' + cookieKey);
        var cookieValue = getCookieValues(cookieKey);
        conditionalLog('Retrieved value for cookie ' + cookieKey + ': ' + cookieValue);

        if (cookieValue == '') {

            conditionalLog('The cookie ' + cookieKey + ' will be used as null here, because its value is empty');


        }

        if (cookieValue != '') {

            updateCookiesFlag = true;
            updatedMarketingMap[cookieKey] = cookieValue;
            conditionalLog('Updating/Adding key: ' + cookieKey + ' with value: ' + cookieValue);

        }
    }

    if (updateCookiesFlag == true) {
        // Rebuild the last_u_ids string
        for (var key in updatedMarketingMap) {
            if (updatedMarketingMap.hasOwnProperty(key)) {
                if (newCookieData) {
                    newCookieData += '|';
                }
                newCookieData += key + '=' + updatedMarketingMap[key];
            }
        }
    }

    return newCookieData || null;
}

// Parse the existing cookie into a map
function parseCookieData(cookieData) {
    var cookieMap = {};
    var cookiePairs = cookieData[0].split('|');
    for (var i = 0; i < cookiePairs.length; i++) {
        var pair = cookiePairs[i].split('=');
        cookieMap[pair[0]] = pair[1];
        conditionalLog('Existing pair found: ' + pair[0] + '=' + pair[1]);
    }
    return cookieMap;
}

// Process query parameter data for initial_c_ids
function processInitialMarketingClickIds(existingInitialQueryParams, clicksList) {
    if (existingInitialQueryParams == '') {
        conditionalLog('No existing initial_c_ids found. Proceeding to fetch query parameters.');
        var newQueryParams = buildNewQueryParams(clicksList);
        if (newQueryParams) {
            setCookie('initial_c_ids', newQueryParams, {
                'max-age': initialCookieLifetime * 24 * 60 * 60,
                'domain': 'auto'
            }, false);
            conditionalLog('New initial_c_ids set successfully.');
        } else {
            conditionalLog('No query parameter values found in clicksList. initial_c_ids will not be set.');
        }
    } else {
        conditionalLog('Existing initial_c_ids found. Proceeding to update it.');
        var updatedQueryParams = updateExistingInitialQueryParams(existingInitialQueryParams, clicksList);


        if (updatedQueryParams) {
            setCookie('initial_c_ids', updatedQueryParams, {
                'max-age': initialCookieLifetime * 24 * 60 * 60,
                'domain': 'auto'
            }, false);
            conditionalLog('Updated initial_c_ids set successfully.');
        } else {
            conditionalLog('No updates made to initial_c_ids. No values found.');
        }
    }
}

// Process query parameter data for last_c_ids
function processLastMarketingClickIds(existingLastQueryParams, clicksList) {
    if (existingLastQueryParams == '') {
        conditionalLog('No existing last_c_ids found. Proceeding to fetch query parameters.');
        var newQueryParams = buildNewQueryParams(clicksList);
        if (newQueryParams) {
            setCookie('last_c_ids', newQueryParams, {
                'max-age': lastCookieLifetime * 24 * 60 * 60,
                'domain': 'auto'
            }, false);
            conditionalLog('New last_c_ids set successfully.');
        } else {
            conditionalLog('No query parameter values found in clicksList. last_c_ids will not be set.');
        }
    } else {
        conditionalLog('Existing last_c_ids found. Proceeding to update it.');
        var updatedQueryParams = updateExistingLastQueryParams(existingLastQueryParams, clicksList);


        if (updatedQueryParams) {
            setCookie('last_c_ids', updatedQueryParams, {
                'max-age': lastCookieLifetime * 24 * 60 * 60,
                'domain': 'auto'
            }, false);
            conditionalLog('Updated last_c_ids set successfully.');
        } else {
            conditionalLog('No updates made to last_c_ids. No values found.');
        }
    }
}

// Build new cookie string
function buildNewQueryParams(clicksList) {
    var newQueryParams = '';
    for (var i = 0; i < clicksList.length; i++) {
        var queryParamKey = clicksList[i].trim();
        conditionalLog('Fetching value for query parameter: ' + queryParamKey);
        var queryParamValue = getQueryParameters(queryParamKey);


        if (!queryParamValue) {

            queryParamValue = '';

        }

        conditionalLog('Retrieved value for query parameter ' + queryParamKey + ': ' + queryParamValue);

        if (queryParamValue == '') {

            conditionalLog('The query param ' + queryParamValue + ' will be used here as null, because its value is empty');

            if (newQueryParams) {
                newQueryParams += '|';
            }
            newQueryParams += queryParamKey + '=' + queryParamValue;

        }

        if (queryParamValue != '') {
            if (newQueryParams) {
                newQueryParams += '|';
            }
            newQueryParams += queryParamKey + '=' + queryParamValue;
        }
    }
    return newQueryParams || null;
}

// Update existing initial_c_ids with new values
function updateExistingInitialQueryParams(existingInitialQueryParams, clicksList) {
    var updatedClickMap = parseQueryParams(existingInitialQueryParams);
    var newQueryData = '';
    var updateParamsFlag = false;

    for (var i = 0; i < clicksList.length; i++) {
        var queryParamKey = clicksList[i].trim();
        conditionalLog('Fetching value for query parameter: ' + queryParamKey);
        var queryParamValue = getQueryParameters(queryParamKey);

        if (!queryParamValue) {

            queryParamValue = '';

        }

        conditionalLog('Retrieved value for query parameter ' + queryParamKey + ': ' + queryParamValue);


        if (queryParamValue == '') {

            conditionalLog('The query param ' + queryParamValue + ' will be used as null here, because its value is empty');


        }

        if (queryParamValue != '') {

            if (updatedClickMap[queryParamKey] == '' || !updatedClickMap[queryParamKey]) {

                updateParamsFlag = true;
                updatedClickMap[queryParamKey] = queryParamValue;
                conditionalLog('Updating/Adding query param: ' + queryParamKey + ' with value: ' + queryParamValue);

            } else {

                conditionalLog('The query param ' + queryParamKey + ' will not be updated, because of initial atribtuion type. The previous value is not empty.');

            }

        }
    }

    if (updateParamsFlag == true) {

        // Rebuild the initial_c_ids string
        for (var key in updatedClickMap) {
            if (updatedClickMap.hasOwnProperty(key)) {
                if (newQueryData) {
                    newQueryData += '|';
                }
                newQueryData += key + '=' + updatedClickMap[key];
            }
        }
    }

    return newQueryData || null;
}

// Update existing last_c_ids with new values
function updateExistingLastQueryParams(existingLastQueryParams, clicksList) {
    var updatedClickMap = parseQueryParams(existingLastQueryParams);
    var newQueryData = '';
    var updateParamsFlag = false;

    for (var i = 0; i < clicksList.length; i++) {
        var queryParamKey = clicksList[i].trim();
        conditionalLog('Fetching value for query parameter: ' + queryParamKey);
        var queryParamValue = getQueryParameters(queryParamKey);

        if (!queryParamValue) {

            queryParamValue = '';

        }

        conditionalLog('Retrieved value for query parameter ' + queryParamKey + ': ' + queryParamValue);


        if (queryParamValue == '') {

            conditionalLog('The query param ' + queryParamValue + ' will be used as null here, because its value is empty');


        }

        if (queryParamValue != '') {

            updateParamsFlag = true;
            updatedClickMap[queryParamKey] = queryParamValue;
            conditionalLog('Updating/Adding query param: ' + queryParamKey + ' with value: ' + queryParamValue);

        }
    }

    if (updateParamsFlag == true) {

        // Rebuild the last_c_ids string
        for (var key in updatedClickMap) {
            if (updatedClickMap.hasOwnProperty(key)) {
                if (newQueryData) {
                    newQueryData += '|';
                }
                newQueryData += key + '=' + updatedClickMap[key];
            }
        }
    }

    return newQueryData || null;
}

// Parse the existing cookie into a map
function parseQueryParams(queryParams) {
    var paramMap = {};
    var paramPairs = queryParams[0].split('|');
    for (var i = 0; i < paramPairs.length; i++) {
        var pair = paramPairs[i].split('=');
        paramMap[pair[0]] = pair[1];
        conditionalLog('Existing query param: ' + pair[0] + '=' + pair[1]);
    }
    return paramMap;
}

// Main function to execute all steps
function processAttributionData() {
    var lists = retrieveLists();
    var initialValues = retrieveExistingInitialValues();
    var lastValues = retrieveExistingLastValues();

    if (data.firstClickAttribution == true) {

        processInitialMarketingCookieIds(initialValues.existingInitialMarketingIds, lists.cookiesList);
        processInitialMarketingClickIds(initialValues.existingInitialQueryParams, lists.clicksList);

    } else {

        conditionalLog('No first click attribution is enabled in the tag. The cookie will not be processed and set.');

    }

    if (data.lastClickAttribution == true) {

        processLastMarketingCookieIds(lastValues.existingLastMarketingIds, lists.cookiesList);
        processLastMarketingClickIds(lastValues.existingLastQueryParams, lists.clicksList);

    } else {

        conditionalLog('No last click attribution is enabled in the tag. The cookie will not be processed and set.');

    }

    // Final Step: Call GTM success callback
    data.gtmOnSuccess();
    conditionalLog('GTM onSuccess called. Script execution complete.');
}

// Execute the function to process the data
processAttributionData();


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "get_cookies",
        "versionId": "1"
      },
      "param": [
        {
          "key": "cookieAccess",
          "value": {
            "type": 1,
            "string": "any"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "set_cookies",
        "versionId": "1"
      },
      "param": [
        {
          "key": "allowedCookies",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "name"
                  },
                  {
                    "type": 1,
                    "string": "domain"
                  },
                  {
                    "type": 1,
                    "string": "path"
                  },
                  {
                    "type": 1,
                    "string": "secure"
                  },
                  {
                    "type": 1,
                    "string": "session"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "initial_u_ids"
                  },
                  {
                    "type": 1,
                    "string": "*"
                  },
                  {
                    "type": 1,
                    "string": "*"
                  },
                  {
                    "type": 1,
                    "string": "any"
                  },
                  {
                    "type": 1,
                    "string": "any"
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "name"
                  },
                  {
                    "type": 1,
                    "string": "domain"
                  },
                  {
                    "type": 1,
                    "string": "path"
                  },
                  {
                    "type": 1,
                    "string": "secure"
                  },
                  {
                    "type": 1,
                    "string": "session"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "initial_c_ids"
                  },
                  {
                    "type": 1,
                    "string": "*"
                  },
                  {
                    "type": 1,
                    "string": "*"
                  },
                  {
                    "type": 1,
                    "string": "any"
                  },
                  {
                    "type": 1,
                    "string": "any"
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "name"
                  },
                  {
                    "type": 1,
                    "string": "domain"
                  },
                  {
                    "type": 1,
                    "string": "path"
                  },
                  {
                    "type": 1,
                    "string": "secure"
                  },
                  {
                    "type": 1,
                    "string": "session"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "last_u_ids"
                  },
                  {
                    "type": 1,
                    "string": "*"
                  },
                  {
                    "type": 1,
                    "string": "*"
                  },
                  {
                    "type": 1,
                    "string": "any"
                  },
                  {
                    "type": 1,
                    "string": "any"
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "name"
                  },
                  {
                    "type": 1,
                    "string": "domain"
                  },
                  {
                    "type": 1,
                    "string": "path"
                  },
                  {
                    "type": 1,
                    "string": "secure"
                  },
                  {
                    "type": 1,
                    "string": "session"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "last_c_ids"
                  },
                  {
                    "type": 1,
                    "string": "*"
                  },
                  {
                    "type": 1,
                    "string": "*"
                  },
                  {
                    "type": 1,
                    "string": "any"
                  },
                  {
                    "type": 1,
                    "string": "any"
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "get_url",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urlParts",
          "value": {
            "type": 1,
            "string": "any"
          }
        },
        {
          "key": "queriesAllowed",
          "value": {
            "type": 1,
            "string": "any"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 27/11/2024, 13:35:52


